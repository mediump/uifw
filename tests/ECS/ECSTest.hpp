...
TEST_CASE("Defaults")
{
  ...
  CHECK_EQ(entity.has<ecs::BaseComponent>(), true);

  const auto component = entity.get<ecs::BaseComponent>();

  CHECK_EQ(component.rect.x, 0.0f);
  CHECK_EQ(component.rect.y, 0.0f);
  CHECK_EQ(component.rect.width, 256.0f);
  CHECK_EQ(component.rect.height, 256.0f);

  CHECK_EQ(component.zOrder, 0.0f);
  CHECK_EQ(component.inLayout, false);
  CHECK_EQ(component.needsUpdate, true);

  const char *entityName = entity.name();
  CHECK_EQ(SDL_strcmp(entityName, ""), 0);
}
...
TEST_CASE("Basic entity")
{
  ...
  // Get and check data
  const auto component = entity.get<ecs::BaseComponent>();
  CHECK_EQ(component.rect.x, X);
  CHECK_EQ(component.rect.y, Y);
  CHECK_EQ(component.rect.width, WIDTH);
  CHECK_EQ(component.rect.height, HEIGHT);
  ...
  CHECK_EQ(mutComponent->rect.x, X_2);
  CHECK_EQ(mutComponent->rect.y, Y_2);
  CHECK_EQ(mutComponent->rect.width, WIDTH_2);
  CHECK_EQ(mutComponent->rect.height, HEIGHT_2);

  // Check name
  const char *entityName = entity.name();
  CHECK_EQ(strcmp(entityName, NAME), 0);

  // Remove component
  entity.remove<ecs::BaseComponent>();
  CHECK_EQ(entity.has<ecs::BaseComponent>(), false);
}
...
TEST_CASE("Multiple components")
{
  ...
  const auto NAME_2 = "Entity2";
  const auto NAME_3 = "Entity3";
  ...
  // Check components
  CHECK_EQ(entity1.has<ecs::BaseComponent>(), true);
  CHECK_EQ(entity2.has<ecs::BaseComponent>(), true);
  CHECK_EQ(entity3.has<ecs::BaseComponent>(), true);
  ...
  CHECK_EQ(strcmp(entity1Name, NAME_1), 0);
  CHECK_EQ(strcmp(entity2Name, NAME_2), 0);
  CHECK_EQ(strcmp(entity3Name, NAME_3), 0);
}
...
TEST_CASE("Transform hierarchy")
{
  ...
  CHECK_EQ(parentRel.nChildren, 3);
  CHECK_EQ(parent.name(), PARENT_NAME);
  ...
  CHECK_EQ(child1Rel.nChildren, 0);
  CHECK_EQ(child1Rel.prev, UI_NULL_ENTITY);
  ...
  CHECK_EQ(child2Rel.nChildren, 0);
  ...
  CHECK_EQ(child3Rel.nChildren, 0);
  CHECK_EQ(child3Rel.prev.name(), child2Entity.name());
  CHECK_EQ(child3Rel.next, UI_NULL_ENTITY);
